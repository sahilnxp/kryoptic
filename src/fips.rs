...
impl ProviderSignatureCtx {
    pub fn new(alg: *const c_char) -> KResult<ProviderSignatureCtx> {
        let sigtable = unsafe {
            EVP_SIGNATURE_fetch(get_libctx(), alg, std::ptr::null())
        };
        if sigtable.is_null() {
            return err_rv!(CKR_DEVICE_ERROR);
        }
        ...
        let ctx = unsafe {
            match (*sigtable).newctx {
                Some(f) => f(FIPS_PROVIDER.provider as *mut c_void, std::ptr::null()),
    ...
    pub fn digest_sign_init(&mut self, mdname: *const c_char, pkey: &EvpPkey, params: *const OSSL_PARAM) -> KResult<()> {
        unsafe {
            match (*self.vtable).digest_sign_init {
                Some(f) => res_to_err!(f(self.ctx, mdname, (*pkey.as_ptr()).keydata as *mut c_void, params)),
    ...
    pub fn digest_sign_update(&mut self, data: &[u8]) -> KResult<()> {
        unsafe {
            match (*self.vtable).digest_sign_update {
                Some(f) => res_to_err!(f(self.ctx, data.as_ptr() as *const c_uchar, data.len())),
                None => err_rv!(CKR_DEVICE_ERROR),
            }
        }
    }
        ...
        unsafe {
            match (*self.vtable).digest_sign_final {
                Some(f) => {
                    ...
                    res_to_err!(f(self.ctx, signature.as_mut_ptr() as *mut c_uchar, siglen_ptr, signature.len()))
                },
                None => err_rv!(CKR_DEVICE_ERROR),
            }
        }
    ...
    pub fn digest_verify_init(&mut self, mdname: *const c_char, pkey: &EvpPkey, params: *const OSSL_PARAM) -> KResult<()> {
        unsafe {
    ...
    pub fn digest_verify_final(&mut self, signature: &[u8]) -> KResult<()> {
        unsafe {
            match (*self.vtable).digest_verify_final {
                Some(f) => res_to_err!(f(self.ctx, signature.as_ptr() as *const c_uchar, signature.len())),
                None => err_rv!(CKR_DEVICE_ERROR),
            }
        }
    }
