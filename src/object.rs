...
pub trait CommonKeyTemplate {
    fn init_common_key_attrs(&mut self) {
        ...
        self.get_template().push(attr_element!(CKA_LOCAL; req false; def true; from_bool; val false));
...
impl ObjectTemplates {
    pub fn new() -> ObjectTemplates {
        ...
        ot.templates.insert(ObjectType::DataObj, Box::new(DataTemplate::new()));
        ot.templates.insert(ObjectType::X509CertObj, Box::new(X509Template::new()));
        ot.templates.insert(ObjectType::RSAPubKey, Box::new(RSAPubTemplate::new()));
        ot.templates.insert(ObjectType::RSAPrivKey, Box::new(RSAPrivTemplate::new()));
        ot
    }
    ...
    pub fn create(&self, handle: CK_ULONG, template: &[CK_ATTRIBUTE]) -> KResult<Object> {
        let mut obj = Object {
            handle: handle,
            attributes: Vec::new(),
        };
