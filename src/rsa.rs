...
impl ObjectTemplate for RSAPrivTemplate {
    fn create(&self, template: &[CK_ATTRIBUTE]) -> KResult<Object> {
        ...
        if p == None || q == None || a == None || b == None || c == None {
            ...
            match p {
                Some(v) => {
                    if RsaDecompose::comp_slice(&r.p, v.as_slice()) != 0 {
                        return err_rv!(CKR_ATTRIBUTE_VALUE_INVALID);
                    }
                }
                None => obj.set_attr(attribute::from_bytes(
                    CKA_PRIME_1,
                    RsaDecompose::to_u8_vec(&r.p),
                ))?,
            }

            match q {
                Some(v) => {
                    if RsaDecompose::comp_slice(&r.q, v.as_slice()) != 0 {
                        return err_rv!(CKR_ATTRIBUTE_VALUE_INVALID);
                    }
                }
                None => obj.set_attr(attribute::from_bytes(
                    CKA_PRIME_2,
                    RsaDecompose::to_u8_vec(&r.q),
                ))?,
            }
            match a {
                Some(v) => {
                    if RsaDecompose::comp_slice(&r.a, v.as_slice()) != 0 {
                        return err_rv!(CKR_ATTRIBUTE_VALUE_INVALID);
                    }
                }
                None => obj.set_attr(attribute::from_bytes(
                    CKA_EXPONENT_1,
                    RsaDecompose::to_u8_vec(&r.a),
                ))?,
            }
            match b {
                Some(v) => {
                    if RsaDecompose::comp_slice(&r.b, v.as_slice()) != 0 {
                        return err_rv!(CKR_ATTRIBUTE_VALUE_INVALID);
                    }
                }
                None => obj.set_attr(attribute::from_bytes(
                    CKA_EXPONENT_2,
                    RsaDecompose::to_u8_vec(&r.b),
                ))?,
            }
            match c {
                Some(v) => {
                    if RsaDecompose::comp_slice(&r.c, v.as_slice()) != 0 {
                        return err_rv!(CKR_ATTRIBUTE_VALUE_INVALID);
                    }
                }
                None => obj.set_attr(attribute::from_bytes(
                    CKA_COEFFICIENT,
                    RsaDecompose::to_u8_vec(&r.c),
                ))?,
            }
        }
...
impl Mechanism for RsaPKCSMechanism {
    ...
    fn generate_keypair(
        ...
        RsaPKCSOperation::generate_keypair(rng, exponent, bits, &mut pubkey, &mut privkey)?;
